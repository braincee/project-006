...
export default function GetEnsAddress() {
    ...
    useEffect(() => {
      ...
      const fetchEnsAddress = async () => {
        try {
          ...
          const address = await publicClient.getEnsAddress({ name: normalizedEnsName });
          setResolvedAddress(address || null);
        } catch (error) {
          console.error('Error retrieving ENS address:', error);
          setResolvedAddress(null);
        }
      };
